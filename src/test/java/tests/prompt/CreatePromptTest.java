...
import api.prompt.CreatePromptApi;
        ...
        for (Report.TestCase tc : cases) {
            ...
            ApiTestExecutor.execute(
                    ...
                    () -> {
                        ...
                        // 4️⃣ Call API (FIXED: added authType)
                        Response response = CreatePromptApi.createPrompt(
                                req,
                                tc.getRole(),
                                tc.getAuthType()
                        );
                        ...
                        if (response.getStatusCode() == 200) {
                            ...
                            Integer promptId =
                                    response.jsonPath().get("insertedId");

                            if (promptId != null) {

                                String promptType =
                                        String.valueOf(req.get("promptType"));
